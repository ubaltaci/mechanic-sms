...
var providers = {
    ...
    "TWILIO": require("./provider/twilio-provider"),
    "ILETIMERKEZI": require("./provider/ileti-merkezi-provider")
};
...
MechanicSms.prototype._phoneNumberCheck = function (phoneNumber) {
    ...
    for (var i = 0; i < phoneNumber.length; i++) {
        ...
        if (!(digit >= "0" && digit <= "9") && (digit == "+" && i != 0)) {
            throw new Error("Only numeric characters and (+) sign as first character allowed in phone number: " + phoneNumber);
        }
    }

    if (phoneNumber.length <= 15) {
        // Phone number should contain country code and its length should be 12
        if ((phoneNumber[0] == "9" && phoneNumber[1] == "0") || (phoneNumber[0]== "+" && phoneNumber[1] == "9" && phoneNumber[2] == "0")) {
            // If phone number starts with 90
            // then we can assume that its a number belongs to Turkey
            if ((phoneNumber[0] == "9" && phoneNumber[1] == "0" && phoneNumber[2] == "5") || (phoneNumber[1] == "9" && phoneNumber[2] == "0" && phoneNumber[3] == "5") ) {
